...
// 4) return true if location, false if no location.
{
  ...
  { // jag 940616
    if (Duvx)
    {
      double aStep = std::abs((BornInf(1) - UVap(1)) / Duvx); // iso U =BornInf(1)
      if (aStep < Step)
        Step = aStep;
    }
  ...
  { // jag 940616
    if (Duvx)
    {
      double aStep = std::abs((BornSup(1) - UVap(1)) / Duvx); // iso U =BornSup(1)
      if (aStep < Step)
        Step = aStep;
    }
  ...
  { // jag 940616
    if (Duvy)
    {
      double aStep = std::abs((BornInf(2) - UVap(2)) / Duvy); // iso V =BornInf(2)
      if (aStep < Step)
        Step = aStep;
    }
    BornSup(2) = BornInf(2);
  ...
  { // jag 940616
    if (Duvy)
    {
      double aStep = std::abs((BornSup(2) - UVap(2)) / Duvy); // iso V =BornSup(2)
      if (aStep < Step)
        Step = aStep;
    }
    BornInf(2) = BornSup(2);
  ...
  for (int i = 1; i < CopySeqAlone.Length(); i++)
  {
    ...
    else // real curve
    {
      ...
      if (Rsnld.IsDone() && std::abs(sp.Root()) <= sp.Tolerance() && !sp.IsTangent())
      {
        ...
        bool     IsPmidValid = (ScalProd1 > 0. && ScalProd2 < 0); // Pmid is in the middle
        if (IsPmidValid)
        {
...

{
  ...
  bool            Arrive     = false; // shows if the line ends
  bool            Cadre      = false; // shows if one is on border of the domain
  bool            ArretAjout = false; // shows if one is on added point
  IntSurf_PntOn2S Psol;
  occ::handle<IntWalk_TheIWLine> CurrentLine; // line under construction
  bool                           Tgtend = false;
...
// ********************************************************************
{
  ...
  double                         PasC       = 0.0;   // step of advancement on the tangent
  double                         PasCu      = 0.0;   // step of advancement current by U
  double                         PasCv      = 0.0;   // step of advancement current by V
  double                         PasSav     = 0.0;   // save first step of advancement
  bool                           Arrive     = false; // show if line ends
  bool                           Cadre      = false; // show if on border of the  domains
  bool                           ArretAjout = false; // show if on the added point
  IntSurf_PntOn2S                Psol;
  occ::handle<IntWalk_TheIWLine> CurrentLine; // line under construction
  ThePointOfPath                 PathPnt;
  ...
  int                      NbDivision = 0; // number of divisions of step
      ...
      {                                                          // as no test of stop is passed
        ...
        if (Rsnld.IsDone())
        {
          ...
          { // no solution for the tolerance
            ...
            if (PasCu <= tolerance(1) && PasCv <= tolerance(2))
            {
              ...
              /*
              Arrive = true;
              CurrentLine->AddStatusFirstLast(false,
                false,false);
              Rajout = true;
              seqAlone.Append(lines.Length()+1);
              seqAjout.Append(lines.Length()+1);
              ...
              for (int aCoordIdx = 1; aCoordIdx <= 2; aCoordIdx++)
              {
                             ...
                             < Precision::PConfusion()))
                {
                  ...
                  if (aStatus != IntWalk_PasTropGrand)
                  {
                    ...
                    int aFixIdx = aCoordIdx == 1 ? 2 : 1; // Fixing index;
                    Uvap(aFixIdx) =
        ...
        { // no numerical solution NotDone
          ...
          if (PasCu <= tolerance(1) && PasCv <= tolerance(2))
          {
            ...
            /*
            Arrive = true;
            CurrentLine->AddStatusFirstLast(false,false,
              false);
            Tgtend = true;
            Rajout = true;
            seqAlone.Append(lines.Length()+1);
