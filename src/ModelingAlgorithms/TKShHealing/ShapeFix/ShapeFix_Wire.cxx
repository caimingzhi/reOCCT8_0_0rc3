...
#include <ShapeFix_IntersectionTool.hpp>
...
void ShapeFix_Wire::ClearStatuses()
{
  ...
  myStatusGaps3d           = emptyStatus;
  myStatusGaps2d           = emptyStatus;
                       ...
                       const bool                        RemoveLoop3d)
{
  ...
  double dist1 = pcurPnt.Distance(crv->Value(Seq3d->Value(1)));
    ...
    for (int iter = 0; iter < 30; iter++)
    {
      ...
      for (int i = 1; i <= points2d.Length(); i++)
      {
        ...
        if (dist21 < tol1 * tol1 || dist22 < tol2 * tol2)
          continue;
        newtol = 1.001 * std::sqrt(std::min(dist21, dist22));
...
bool ShapeFix_Wire::FixIntersectingEdges(const int num)
{
  ...
  for (int i = 1; i <= nb; i++)
  {
    ...
    if (locMayEdit || newtol <= MaxTolerance())
    {
      ...
      if (locMayEdit && newRange1 <= prevRange1 && newRange2 <= prevRange2
...
bool ShapeFix_Wire::FixLacking(const int num, const bool force)
{
  ...
  bool doAddLong   = false;
  bool doAddClosed = false;
  bool doAddDegen  = false;
  bool doBend      = false;
  ...
  else if (!BRep_Tool::Degenerated(E2) && !BRep_Tool::Degenerated(E1))
  {
    ...
    if (myTopoMode)
    {
      ...
      double tol0    = tol1 + tol2;
      double dist3d2 = p3d1.SquareDistance(p3d2);

      if (!myAnalyzer->LastCheckStatus(ShapeExtend_DONE2) && dist3d2 > 1.25 * tol0 * tol0
