...
GeomAbs_Shape ChFi3d_evalconti(const TopoDS_Edge&, const TopoDS_Face& F1, const TopoDS_Face& F2)
{
                        ...
                        const ::model::adapter::BRepAdaptor_Surface&       S2)
{
  ...
  aPA = math::precision::Precision::Angular();
                      ...
                      const bool                          enlarge)
{
  ...
  if ((last - first) < math::precision::Precision::PConfusion())
    return false;
  if (enlarge)
  ...
  {
    Intersection.Perform(C1, math::precision::Precision::PIntersection(), math::precision::Precision::PIntersection());
  }
  else
  {
    Intersection.Perform(C1, C2, math::precision::Precision::PIntersection(), math::precision::Precision::PIntersection());
  }
  if (Intersection.IsDone())
  {
...
void ChFi3d_ReparamPcurv(const double Uf, const double Ul, occ::handle<Geom2d_Curve>& Pcurv)
{
  ...
  if (pc.IsNull())
    return;
  if (std::abs(upcf - pc->FirstParameter()) > math::precision::Precision::PConfusion()
      || std::abs(upcl - pc->LastParameter()) > math::precision::Precision::PConfusion())
  {
    pc->Segment(upcf, upcl);
  }
  if (std::abs(Uf - pc->FirstParameter()) > math::precision::Precision::PConfusion()
      || std::abs(Ul - pc->LastParameter()) > math::precision::Precision::PConfusion())
  {
                          ...
                          double&                          tolreached)
{
  ...
  occ::handle<GeomAdaptor_Curve>   hc(new GeomAdaptor_Curve(C3d, Pardeb, Parfin));
...
void ChFi3d_ComputePCurv(const gp_Pnt2d&            UV1,
                         ...
                         const bool                 reverse)
{
  const double tol = math::precision::Precision::PConfusion();
  gp_Pnt2d     p1, p2;
  if (!reverse)
  {
    p1 = UV1;
...

{
  ...
  if (std::abs(UV1.X() - UV2.X()) <= tol2d)
  {
    ...
    if (IFlag != 1)
    {
      ...
      const occ::handle<Adaptor3d_Curve>& aHCurve = hc;
  ...
  else if (IFlag == 0)
  {
    ...
    else
    {
      ...
      bs.SetGap(math::precision::Precision::PConfusion());
      bool aIN = true;
  ...
  else
  {
    ...
    if (!UV1.IsEqual(p2d, math::precision::Precision::PConfusion()))
    {
                                           ...
                                           const double               Tol)
{
  ...
  CP.SetPoint(SP.Value());
  if (SP.HasTangent())
  {
    if (Start)
    {
      CP.SetVector(SP.Tangent().Reversed());
    }
    else
    {
  ...
  CP.SetParameter(SP.ParameterOnGuide());
                  ...
                  const double                      tol2d)
{
  ...
  ChFiDS_Regul regcout;
  ChFiDS_Regul regfilfil;
  ...
  if (SeqFil.Length() > 1)
  {
    for (j = 1; j <= SeqFil.Length(); j++)
    {
      ...
      for (onS = 1; onS <= 2; onS++)
      {
        ...
        if (FiLen > math::precision::Precision::PConfusion())
          continue;
  ...
  for (iPart1 = 1; iPart1 <= aSurDat1->Length(); iPart1++)
  {
    ...
    for (iPart2 = 1; iPart2 <= aSurDat2->Length(); iPart2++)
    {
      ...
      Geom2dAdaptor_Curve aPCurve2(aFI2.PCurveOnFace(),
                                   aFI2.FirstParameter(),
                                   aFI2.LastParameter());
      anIntersector.Perform(aPCurve1, aPCurve2, tol2d, math::precision::Precision::PConfusion());
                             ...
                             const occ::handle<Geom_Curve>&        C)
{
  ...
  distmax = std::max(distmax, math::precision::Precision::Confusion());
  return distmax;
}

occ::handle<Geom_Surface> trsfsurf(const occ::handle<Adaptor3d_Surface>& HS,
                                   occ::handle<Adaptor3d_TopolTool>&)
{
                          ...
                          const bool                            wholeCurv)
{
  ...
  double distrefdeb = pdeb1.Distance(pdeb2);
  double distreffin = pfin1.Distance(pfin2);
  if (distrefdeb < tol3d)
    distrefdeb = tol3d;
  if (distreffin < tol3d)
    distreffin = tol3d;
      ...
      || (S1->GetType() == GeomAbs_Plane && S2->GetType() == GeomAbs_Cylinder))
  {
    ...
    IntAna_QuadQuadGeo ImpKK(pl, cyl, math::precision::Precision::Angular(), tol3d);
    ...
    if (isIntDone)
    {
      ...
      if (!c1line)
        ElCLib::AdjustPeriodic(0., 2 * M_PI, math::precision::Precision::Angular(), Udeb, Ufin);
      occ::handle<GeomAdaptor_Curve> HC = new GeomAdaptor_Curve();
  ...
  {
    IntAna_QuadQuadGeo LInt(S1->Plane(), S2->Plane(), math::precision::Precision::Angular(), tol3d);
    if (LInt.IsDone())
    {
        ...
        for (int ilin = 1; ilin <= nbl; ilin++)
        {
          if (inter.HasLineOnS1(ilin) && inter.HasLineOnS2(ilin))
          {
            ...
            else
            {
              ...
              proj.Init(pdeb1, C3d);
              if (proj.NbPoints() == 0 && distrefdeb > math::precision::Precision::Confusion())
                proj.Perform(pdeb2);
              if (proj.NbPoints() == 0)
                failedF = true;
              else
              ...
              proj.Perform(pfin1);
              if (proj.NbPoints() == 0 && distreffin > math::precision::Precision::Confusion())
                proj.Perform(pfin2);
              if (proj.NbPoints() == 0)
                failedL = true;
              else
              ...
              {
                if (std::abs(Uf - Ul) < math::precision::Precision::PConfusion())
                  continue;
              }
              ptestdeb = C3d->Value(Uf);
              ptestfin = C3d->Value(Ul);
  ...
  for (;;)
  {
    ...
    else
    {
      ...
      double ddeb = math::precision::Precision::Infinite();
      double dfin = math::precision::Precision::Infinite();
      double dd;
      int    indd = 0, indf = 0;
      for (i = 1; i <= nbp; i++)
      {
