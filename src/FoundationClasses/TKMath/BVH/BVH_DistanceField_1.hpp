...
void BVH_DistanceField<T, N>::BuildSlices(BVH_Geometry<T, N>& theGeometry,
                                          ...
                                          const int           theFinalSlice)
{
  for (int aZ = theStartSlice; aZ < theFinalSlice; ++aZ)
  {
    for (int aY = 0; aY < myDimensionY; ++aY)
    {
      for (int aX = 0; aX < myDimensionX; ++aX)
      {
        BVH_VecNt aCenter;
...
bool BVH_DistanceField<T, N>::Build(BVH_Geometry<T, N>& theGeometry)
{
  ...
  const T aMaxBoxSide =
    std::max(std::max(aGlobalBoxSize.x(), aGlobalBoxSize.y()), aGlobalBoxSize.z());

  myDimensionX = static_cast<int>(myMaximumSize * aGlobalBoxSize.x() / aMaxBoxSide);
  myDimensionY = static_cast<int>(myMaximumSize * aGlobalBoxSize.y() / aMaxBoxSide);
  myDimensionZ = static_cast<int>(myMaximumSize * aGlobalBoxSize.z() / aMaxBoxSide);

  myDimensionX = std::min(myMaximumSize, std::max(myDimensionX, 16));
  myDimensionY = std::min(myMaximumSize, std::max(myDimensionY, 16));
  myDimensionZ = std::min(myMaximumSize, std::max(myDimensionZ, 16));

  const BVH_VecNt aGlobalBoxMin = theGeometry.Box().CornerMin();
  const BVH_VecNt aGlobalBoxMax = theGeometry.Box().CornerMax();

  const int aVoxelOffset = 2;
