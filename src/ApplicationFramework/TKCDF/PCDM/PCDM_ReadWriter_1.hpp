...
public:
  ...
  Standard_EXPORT void WriteExtensions(const occ::handle<app::storage::Storage_Data>& aData,
                                       const occ::handle<CDM_Document>& aDocument) const override;

  Standard_EXPORT void WriteVersion(const occ::handle<app::storage::Storage_Data>& aData,
                                    const occ::handle<CDM_Document>& aDocument) const override;

  Standard_EXPORT int ReadReferenceCounter(
    const TCollection_ExtendedString&     aFileName,
    const occ::handle<System::log::Message_Messenger>& theMsgDriver) const override;
