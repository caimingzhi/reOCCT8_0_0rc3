...
bool AppParCurves_Function::Value(const math_Vector& X, double& F)
{
    ...
    for (Ci = 1; Ci <= NbCu; Ci++)
    {
      ...
      for (i = Adeb; i <= Afin; i++)
      {
        ...
        if (dimen == 3)
        {
          ...
          MyF(i, Ci) += FZ * FZ;
          Fi = MyF(i, Ci);
          if (std::sqrt(Fi) > ERR3d)
            ERR3d = std::sqrt(Fi);
        }
        else
        {
          Fi = MyF(i, Ci);
          if (std::sqrt(Fi) > ERR2d)
            ERR2d = std::sqrt(Fi);
        }
        FVal += Fi;
      }
...
void AppParCurves_Function::Perform(const math_Vector& X)
{
  ...
  else
  {
    int                     Pi, Ci, i, k, dimen;
    int                     Npol = Degre + 1;
    double                  Scal, AA, BB, CC, DAA, DBB, DCC;
    double                  FX, FY, FZ, AIJ, DAIJ, px, py, pz, Fi;
    AppParCurves_Constraint Cons = AppParCurves_NoConstraint;
...
int AppParCurves_Function::NbVariables() const
{
  return NbP;
}

bool AppParCurves_Function::Gradient(const math_Vector& X, math_Vector& G)
{
...
bool AppParCurves_Function::Values(const math_Vector& X, double& F, math_Vector& G)
{
...
double AppParCurves_Function::Error(const int IPoint, const int CurveIndex) const
{
  return std::sqrt(MyF(IPoint, CurveIndex));
}

double AppParCurves_Function::MaxError3d() const
{
  return ERR3d;
}

double AppParCurves_Function::MaxError2d() const
{
  return ERR2d;
}
