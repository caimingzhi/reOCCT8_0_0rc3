                    ...
                    NbPol)
{
  ...
  NbCu   = nb3d + nb2d;
  tabdim = new NCollection_HArray1<int>(0, NbCu - 1);
...
bool AppParCurves_BSpFunction::Value(const math_Vector& X, double& F)
{
...
void AppParCurves_BSpFunction::SetFirstLambda(const double l1)
{
  mylambda1 = l1;
}

void AppParCurves_BSpFunction::SetLastLambda(const double l2)
{
  mylambda2 = l2;
}

int AppParCurves_BSpFunction::NbVariables() const
{
  return NbP;
}

bool AppParCurves_BSpFunction::Gradient(const math_Vector& X, math_Vector& G)
{
...
bool AppParCurves_BSpFunction::Values(const math_Vector& X, double& F, math_Vector& G)
{
...
double AppParCurves_BSpFunction::Error(const int IPoint, const int CurveIndex)
{
  const math_Matrix& DD = MyLeastSquare.Distance();
  double             d  = DD.Value(IPoint, CurveIndex);
  if (!Contraintes)
    return d;
  else
    return std::sqrt(MyF(IPoint, CurveIndex));
}

double AppParCurves_BSpFunction::MaxError3d() const
{
  return ERR3d;
}

double AppParCurves_BSpFunction::MaxError2d() const
{
  return ERR2d;
}
