...
#include <GeomLib_DenominatorMultiplierPtr.hpp>
...
void GeomLib_CurveOnSurfaceEvaluator::Evaluate(int*,
                                               double  DebutFin[2],
                                               double* Parameter,
                                               int*    DerivativeRequest,
                                               double* Result,
                                 ...
                                 const bool                      After)
{
  ...
  PLib::HermiteCoefficients(0, 1, Continuity, 0, MatCoefs);
                                 ...
                                 const bool                        After)
{
  ...
  if (BS.IsNull())
  {
    ...
    int                              nmax    = 16;
    int                              thePrec = 1;
    const occ::handle<Geom_Surface>& aSurf   = Surface;
...

{
  ...
  newknots->SetValue(i, knotinserted);
...

{
  ...
  if (CanBeTreated(BSurf))
  {
    for (i = 1; i <= BSurf->NbVPoles(); i++)
    {
      ...
      BCurve = new Geom_BSplineCurve(BCurvePoles,
                                     BCurveWeights,
                                     BCurveKnots,
                                     BCurveMults,
                                     BSurf->UDegree());
    ...
    FunctionMultiply(BSurf, uknotmin, uknotmax);
