...
def generate_namespace_wrapper(namespace_str):
    ...
    若 namespace_str 以 '::' 开头（如 '::core'），包裹时忽略开头的 '::'。
    """
    # 包裹时去掉开头的 ::
...
def process_file(filepath, ref_regex, mapping, mapping_names_set, dry_run):
    ...
    for kw_match in class_keyword_regex.finditer(content):
        ...
        # friend class ClassName → 检查与外层 class 的 namespace 是否相同
        if is_friend:
            friend_ns = mapping[class_name]
            # 查找外层 class（从 protected_body_ranges 中找包含当前位置的 class body）
            enclosing_ns = None
            for (bs, be, cn) in protected_body_ranges:
                if bs <= kw_match.start() <= be:
                    enclosing_ns = mapping.get(cn)
                    break
            if enclosing_ns is not None and enclosing_ns == friend_ns:
                # 同一 namespace → 保留原样，保护类名不被阶段2替换
                protected_name_pos.append((class_name_start, class_name_end))
            # 不同 namespace 或外层 class 不在映射中 → 交给阶段2做前缀替换
            continue
    ...
    for ref_match in ref_regex.finditer(content):
        ...
        # 检查当前引用是否在某个已知 namespace 的 class body 内
        # 如果是，且目标引用的类(name)与外层类(cn)属于同一个 namespace，则跳过添加前缀
        target_ns = mapping[name]
        is_same_ns_inside = False
        for s, e, cn in protected_body_ranges:
            if s <= name_start <= e:
                enclosing_ns = mapping.get(cn)
                if enclosing_ns == target_ns:
                    is_same_ns_inside = True
                    break

        if is_same_ns_inside:
            continue
